...
for i in range(50):
    ...
    for j in range(11):
        ...
        while not done:
            ...
            if test_myenv.delta_psi_decision >= delta_psi_decision_max and s_ != s and float(s_) != 123:
                a = state_action[float(s_)]
                num_psi += 1
            s = s_
    ...
    np.save('./alpha/delta_psi_decision_max_{:.2f}.npy'.format(delta_psi_decision_max), delay_alpha)
    np.save('./alpha/num_psi_alpha_{:.2f}.npy'.format(delta_psi_decision_max), num_psi_alpha)
    np.save('./alpha/num_1_alpha_{:.2f}.npy'.format(delta_psi_decision_max), num_1_alpha)
    np.save('./alpha/num_all_alpha_{:.2f}.npy'.format(delta_psi_decision_max), num_all_alpha)
